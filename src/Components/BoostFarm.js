...
const BoostFarm = () => {
  const { id, setPlusMiningPower, initialMiningPower } = useUser();
  ...
  const [miningPowerList, setMiningPowerList] = useState(miningPowers);
  useEffect(() => {
    setMiningPowerList(miningPowers.map(item => ({...item, capacity: initialMiningPower * item.capacity})));
  }, [initialMiningPower]);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      await updateDoc(userRef, {
        plusMiningPower: newMiningPower,
      });
      setPlusMiningPower(newMiningPower);
              ...
              <div className="w-full flex taskbg rounded-tl-[40px] rounded-tr-[40px] mt-[2px] justify-center relative flex-col items-center space-y-3 p-4 pt-20 pb-10">
                ...
                <div className="w-full flex justify-center flex-col items-center">
                  ...
                  <div className='w-full flex justify-center items-center space-x-2 pb-3'>
                    ...
                    <div className="w-[45%] bg-cards text-[12px] rounded-[6px] p-2 text-primary flex items-center justify-center space-x-1 font-semibold text-center">
                      <span> Profit:</span> <span className='pl-1'><img src='eggs.webp' alt='dfd' className='w-[10px] h-[10px] mt-[2px]' /></span> <span className='text-green-500 '>{selectedPower.capacity} PPH</span>
                    </div>
                  </div>
