...
export const UserProvider = ({ children }) => {
  ...
  const fetchData = async (userId) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        setLastFarmDate(userData.lastFarmDate.toDate());
        setIsFarming(userData.isFarming);
        setCanClaim(userData.canClaim);
        ...
        // Fetch followTasks
        const followTasksQuerySnapshot = await getDocs(collection(db, 'followTasks'));
        const followTasksData = followTasksQuerySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  return (
    <UserContext.Provider value={{ balance, setBalance, openInfoThree, setOpenInfoThree, youtubeTasks, setYoutubeTasks, userYoutubeTasks, setUserYoutubeTasks, followTasks, setFollowTasks, userFollowTasks, setUserFollowTasks, fullName, selectedExchange, setSelectedExchange, leaderBoard, tonTasks, setTonTasks, username, tonTransactions, setTonTransactions, activeMineUserRank, setActiveMineUserRank, setUsername, activeUserRank, setActiveUserRank, mineLeaderBoard, setMineLeaderBoard, miningTotal, setMiningTotal, setLeaderBoard, plusMiningPower, setPlusMiningPower, loadingTwo, setLoadingTwo, checkinRewards, setCheckinRewards, taskPoints, setTaskPoints, slotPoints, setSlotPoints, premiumReward, setPremiumReward, showStartOverModal, setShowStartOverModal, showClaimModal, setShowClaimModal, lastCheckIn, setLastCheckIn, checkInDays, setCheckInDays, error, setError, checker, setChecker, premium, setPremium, userAdvertTasks, setUserAdvertTasks, advertTasks, setAdvertTasks, setFullName, walletAddress, setWalletAddress, isAddressSaved, setIsAddressSaved, loading, setLoading, id, setId, sendUserData, refBonus, setRefBonus, manualTasks, setManualTasks, userManualTasks, setUserManualTasks, tasks, setTasks, completedTasks, setCompletedTasks, referrals, claimedReferralRewards, setClaimedReferralRewards, initialized, setInitialized, lastFarmDate, setLastFarmDate, isFarming, setIsFarming, canClaim, setCanClaim, plusMiningTime, setPlusMiningTime, miningCountPerDay, initialMiningPower, initialMiningTime }}>
      {children}
